...
import profileFakerFactory from "./profileFakerFactory";
...
const ProfileFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = profileFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("profile").create(elem));
...
export default connect(mapState, mapDispatch)(ProfileFakerDialogComponent);
