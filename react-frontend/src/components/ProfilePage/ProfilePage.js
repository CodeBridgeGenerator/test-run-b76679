...
import ProfileCreateDialogComponent from "./ProfileCreateDialogComponent";
import ProfileFakerDialogComponent from "./ProfileFakerDialogComponent";
import ProfileSeederDialogComponent from "./ProfileSeederDialogComponent";
...
const ProfilePage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                console.log({ error });
                setLoading(false);
                props.alert({ title: "Profile", type: "error", message: error.message || "Failed get Profile" });
            });

    const service = _.find(config.services, { serviceName : "profile"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("profile").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Profile", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("profile").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Profile </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="profile-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="profile-datatable">
                    <ProfileDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <ProfileEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ProfileCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <ProfileFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ProfileSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
